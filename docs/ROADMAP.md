...
| 6 | Demonstration-to-policy integration | In progress; steps 1-5 done 2026-08-24, step 6 planned 2026-08-25 but not implemented |
...
6. Add expert demonstration generation. **Planned; implementation requires review.** The
   initial implementation will use a deterministic state-machine expert for
   `franka-pick-place` with the canonical instruction `Pick up the cube and place it in the
   goal.` It will collect one episode per environment and record explicit task, instruction,
...
- Stage 6 step 6 expert generation is documented but remains unimplemented until its metadata,
  expert interface, state-machine phases, and collection lifecycle are reviewed.
